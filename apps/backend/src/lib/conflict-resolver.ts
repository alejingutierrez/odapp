...
export class ConflictResolver {
  async detectProductConflict(localProduct: Record<string, unknown>, shopifyProduct: ShopifyProduct): Promise<ProductConflict | null> {
    ...
    if (Math.abs(localUpdated.getTime() - shopifyUpdated.getTime()) > 60000) { // 1 minute threshold
  ...
  async resolveProductConflict(conflict: ProductConflict): Promise<ConflictResolution> {
    const { localProduct, shopifyProduct, conflictFields, conflictType } = conflict;
    ...
    if (this.canMergeProductConflict(conflictFields)) {
      const mergedData = await this.mergeProductData(localProduct, shopifyProduct, conflictFields);
      return {
        action: 'merge',
        mergedData,
  ...
  async detectCustomerConflict(localCustomer: Record<string, unknown>, shopifyCustomer: ShopifyCustomer): Promise<CustomerConflict | null> {
    ...
    if (localCustomer.email === shopifyCustomer.email && localCustomer.shopifyId !== shopifyCustomer.id.toString()) {
  ...
  async resolveCustomerConflict(conflict: CustomerConflict): Promise<ConflictResolution> {
    const { localCustomer, shopifyCustomer, conflictFields, conflictType } = conflict;
    ...
    if (conflictType === 'duplicate') {
      ...
      const mergedData = await this.deduplicateCustomers(localCustomer, shopifyCustomer);
      return {
        action: 'merge',
        mergedData,
    ...
    if (this.canMergeCustomerConflict(conflictFields)) {
      const mergedData = await this.mergeCustomerData(localCustomer, shopifyCustomer, conflictFields);
      return {
        action: 'merge',
        mergedData,
  ...
  private async mergeProductData(localProduct: Record<string, unknown>, shopifyProduct: ShopifyProduct, conflictFields: string[]): Promise<Record<string, unknown>> {
    ...
    merged.shopifyUpdatedAt = new Date(shopifyProduct.updated_at);
  ...
  private async mergeCustomerData(localCustomer: Record<string, unknown>, shopifyCustomer: ShopifyCustomer, conflictFields: string[]): Promise<Record<string, unknown>> {
    ...
    merged.shopifyUpdatedAt = new Date(shopifyCustomer.updated_at);
  ...
  private async deduplicateCustomers(localCustomer: Record<string, unknown>, shopifyCustomer: ShopifyCustomer): Promise<Record<string, unknown>> {
    ...
    logger.info(`Deduplicated customer: ${localCustomer.email} (local: ${localCustomer.id}, shopify: ${shopifyCustomer.id})`);
  ...
  private mapShopifyCustomerToLocal(shopifyCustomer: ShopifyCustomer): Record<string, unknown> {
    return {
      ...
      shopifyId: shopifyCustomer.id.toString(),
      shopifyUpdatedAt: new Date(shopifyCustomer.updated_at),
      addresses: shopifyCustomer.addresses?.map(address => ({
