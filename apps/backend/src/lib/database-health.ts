...
export class DatabaseHealthChecker {
  ...
  private async checkConnection(): Promise<HealthCheck> {
    ...
    try {
      ...
      return {
        ...
        duration,
        message: duration > 1000 ? 'Connection is slow' : 'Connection is healthy',
      }
    } catch (error) {
      return {
        status: 'fail',
  ...
  private async checkPerformance(): Promise<HealthCheck> {
    ...
    try {
      ...
      return {
        ...
        duration,
        message: duration > this.slowQueryThreshold 
  ...
  private async getMetrics(): Promise<DatabaseMetrics> {
    try {
      ...
      const connections = connectionStats.reduce((acc, stat) => {
