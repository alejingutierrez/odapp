...
import { authenticate, authRateLimit, requireTwoFactor } from '../middleware/auth'
 ...
 * Register a new user
 */
router.post('/register', authRateLimit(10, 60), registerValidation, async (req: Request, res: Response) => {
  try {
 ...
 * Login user
 */
router.post('/login', authRateLimit(10, 15), loginValidation, async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    if (twoFactorEnabled) {
      ...
      if (twoFactorToken) {
        isTwoFactorValid = await TwoFactorService.verifyUserTwoFactor(user.id, twoFactorToken)
      }

      // If TOTP failed, try backup code
      if (!isTwoFactorValid && backupCode) {
        isTwoFactorValid = await TwoFactorService.verifyBackupCode(user.id, backupCode)
      }
 ...
 */
router.post('/forgot-password', authRateLimit(5, 60), forgotPasswordValidation, async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
 ...
 */
router.post('/reset-password', authRateLimit(5, 60), resetPasswordValidation, async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
 ...
 */
router.post('/change-password', authenticate, requireTwoFactor, changePasswordValidation, async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({
