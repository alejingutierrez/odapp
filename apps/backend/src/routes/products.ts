...
router.post('/',
  ...
  async (req, res, next) => {
    try {
      ...
      const response = ApiResponse.success({ product: newProduct }, 'Product created successfully', 201)
...
router.put('/:id',
  ...
  async (req, res, next) => {
    try {
      ...
      const updatedProduct = await productService.updateProduct(id, updateData, req.user?.id)
      ...
      const response = ApiResponse.success({ product: updatedProduct }, 'Product updated successfully')
      res.json(response)
    } catch (error) {
      next(error)
    }
...
router.post('/bulk-update',
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await productService.bulkUpdateProducts(bulkUpdateData, req.user?.id)
...
router.delete('/bulk-delete',
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await productService.bulkDeleteProducts(bulkDeleteData, req.user?.id)
    ...
    try {
      ...
      if (!files || files.length === 0) {
        return res.status(400).json(
          ApiResponse.error('No images provided', 400)
        )
      }
      ...
      const updatedProduct = await productService.updateProduct(id, {
        images: [...(product.images || []), ...imageData]
      }, req.user?.id)
...
router.get('/:id/variants',
  ...
  async (req, res, next) => {
    try {
      ...
      const response = ApiResponse.success({ variants: product.variants })
      res.json(response)
    } catch (error) {
      next(error)
    }
...
router.get('/analytics/overview',
  ...
  async (req, res, next) => {
    try {
      logger.info('Fetching product analytics overview', { userId: req.user?.id })
...
router.get('/analytics/performance',
  ...
  async (req, res, next) => {
    try {
      ...
      const response = ApiResponse.success({ report })
      res.json(response)
    } catch (error) {
      next(error)
    }
...
router.post('/import/csv',
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await importExportService.importProductsFromCSV(csvContent, {
...
router.post('/import/json',
  ...
  async (req, res, next) => {
    try {
      ...
      if (!file) {
        return res.status(400).json(
          ApiResponse.error('JSON file is required', 400)
        )
      }
      ...
      const result = await importExportService.importProductsFromJSON(jsonContent, {
        updateExisting: updateExisting === 'true',
        skipInvalid: skipInvalid === 'true',
        validateOnly: validateOnly === 'true'
      })
...
router.post('/export',
  ...
  async (req, res, next) => {
    try {
      ...
      const response = ApiResponse.success(result, 'Export completed successfully')
      res.json(response)
    } catch (error) {
      next(error)
    }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!q || typeof q !== 'string') {
        return res.status(400).json(
          ApiResponse.error('Query parameter "q" is required', 400)
        )
      }
...
router.delete('/:id/images/:imageId',
  ...
  async (req, res, next) => {
    try {
      ...
      await productService.updateProduct(id, { images: updatedImages }, req.user?.id)
      
      const response = ApiResponse.success(null, 'Image deleted successfully')
      res.json(response)
    } catch (error) {
      next(error)
    }
...
const validateProductAvailability = async (req: any, res: any, next: any) => {
  try {
    ...
    if (!hasInventory) {
      return res.status(400).json(
        ApiResponse.error('At least one variant must have inventory available', 400)
      )
    }
...
router.post('/with-inventory-check',
  validate({ body: createProductSchema }),
