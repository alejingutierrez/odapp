...
export class AnalyticsService {
  ...
  async getProductAnalytics(
    ...
    // Try cache first
    const cached = await this.cache.get<ProductAnalyticsData>(cacheKey)
    if (cached) {
      return cached
    }
    ...
    try {
      ...
      const variantStats = new Map<string, { orders: number; revenue: number; name: string }>()
      
      orderItems.forEach(item => {
        if (item.variantId) {
          const existing = variantStats.get(item.variantId) || { orders: 0, revenue: 0, name: item.variant?.name || 'Unknown' }
          existing.orders += 1
  ...
  async getProductPerformanceReport(
    ...
    // Try cache first
    const cached = await this.cache.get<ProductPerformanceReport[]>(cacheKey)
    if (cached) {
      return cached
    }
    ...
    try {
      ...
      for (const product of products) {
        ...
        const currentStock = product.variants.reduce((sum, variant) => 
          sum + variant.inventory.reduce((invSum, inv) => invSum + inv.quantity, 0), 0
        )
        ...
        const daysInPeriod = filters.startDate && filters.endDate 
          ...
          : 30
        const turnoverRate = currentStock > 0 ? (totalQuantitySold / currentStock) * (365 / daysInPeriod) : 0

        // Calculate trends (compare with previous period)
        const previousPeriodStart = filters.startDate 
          ? new Date(filters.startDate.getTime() - (daysInPeriod * 24 * 60 * 60 * 1000))
          : new Date(Date.now() - (60 * 24 * 60 * 60 * 1000)) // 60 days ago

        const previousPeriodEnd = filters.startDate || new Date(Date.now() - (30 * 24 * 60 * 60 * 1000))
        ...
        const previousRevenue = previousOrderItems.reduce((sum, item) => sum + Number(item.totalPrice), 0)
        ...
        const ordersTrend = previousOrders > 0 ? ((totalOrders - previousOrders) / previousOrders) * 100 : 0
        ...
        reports.push({
          ...
          stockValue,
          turnoverRate,
          daysInStock: Math.ceil((Date.now() - product.createdAt.getTime()) / (1000 * 60 * 60 * 24)),
      ...
      await this.cache.set(cacheKey, reports, 1800)

      return reports
    } catch (error) {
      logger.error('Failed to get product performance report', { error, filters })
      throw error
    }
  }
  ...
  async getCategoryAnalytics(startDate?: Date, endDate?: Date): Promise<CategoryAnalytics[]> {
    const cacheKey = `analytics:categories:${startDate?.toISOString()}:${endDate?.toISOString()}`
    
    // Try cache first
    const cached = await this.cache.get<CategoryAnalytics[]>(cacheKey)
    if (cached) {
      return cached
    }
    ...
    try {
      ...
      const analytics: CategoryAnalytics[] = categories.map(category => {
        ...
        const averagePrice = productCount > 0 
          ? category.products.reduce((sum, product) => sum + Number(product.price), 0) / productCount
          : 0
  ...
  async getInventoryAnalytics(): Promise<InventoryAnalytics> {
    ...
    // Try cache first
    const cached = await this.cache.get<InventoryAnalytics>(cacheKey)
    if (cached) {
      return cached
    }
    ...
    try {
      ...
      const productStockStatus = new Map<string, { inStock: boolean; lowStock: boolean; outOfStock: boolean }>()
      ...
      inventoryItems.forEach(item => {
        ...
        const existing = productStockStatus.get(productId) || { inStock: false, lowStock: false, outOfStock: true }
      ...
      products.forEach(product => {
        ...
        const totalSold = product.variants.reduce((sum, variant) => 
          sum + variant.orderItems.reduce((itemSum, item) => itemSum + item.quantity, 0), 0
        )

        const turnoverRate = totalStock > 0 ? (totalSold / totalStock) * (365 / 90) : 0 // Annualized
        const daysInStock = Math.ceil((Date.now() - product.createdAt.getTime()) / (1000 * 60 * 60 * 24))
      ...
      slowMovingProducts.sort((a, b) => b.daysInStock - a.daysInStock).splice(10)
      fastMovingProducts.sort((a, b) => b.turnoverRate - a.turnoverRate).splice(10)
  ...
  async getSalesAnalytics(
    ...
    // Try cache first
    const cached = await this.cache.get<SalesAnalytics>(cacheKey)
    if (cached) {
      return cached
    }
    ...
    try {
      ...
      const salesTrend = this.generateSalesTrend(orders, period, startDate, endDate)
