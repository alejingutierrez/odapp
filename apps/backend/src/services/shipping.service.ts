...
export class FedExCarrier implements ShippingCarrier {
  ...
  async getRates(request: ShippingRateRequest): Promise<ShippingRate[]> {
    try {
      ...
      if (request.serviceTypes?.length) {
        return rates.filter(rate => request.serviceTypes!.includes(rate.service));
      }

      return rates;
    } catch (error) {
      logger.error('FedEx rate request failed', { error: error.message, request });
      throw new AppError('Failed to get FedEx rates', 500);
    }
  }
  ...
  async createShipment(request: ShipmentRequest): Promise<ShipmentResponse> {
    try {
      ...
      logger.error('FedEx shipment creation failed', { error: error.message, request });
      return {
        success: false,
        rate: 0,
        currency: 'USD',
  ...
  async trackShipment(trackingNumber: string): Promise<TrackingInfo> {
    try {
      ...
      return {
        ...
        deliveredAt: currentStatus === TrackingStatus.DELIVERED ? new Date() : undefined,
        events
      };
    } catch (error) {
      logger.error('FedEx tracking failed', { error: error.message, trackingNumber });
      throw new AppError('Failed to track FedEx shipment', 500);
    }
  }
  ...
  async cancelShipment(shipmentId: string): Promise<boolean> {
    try {
      ...
      logger.error('FedEx shipment cancellation failed', { error: error.message, shipmentId });
      return false;
    }
  }
}
...
export class UPSCarrier implements ShippingCarrier {
  ...
  async getRates(request: ShippingRateRequest): Promise<ShippingRate[]> {
    try {
      ...
      logger.error('UPS rate request failed', { error: error.message, request });
      throw new AppError('Failed to get UPS rates', 500);
    }
  }
  ...
  async createShipment(request: ShipmentRequest): Promise<ShipmentResponse> {
    try {
      ...
      logger.error('UPS shipment creation failed', { error: error.message, request });
      return {
        success: false,
        rate: 0,
        currency: 'USD',
  ...
  async trackShipment(trackingNumber: string): Promise<TrackingInfo> {
    try {
      ...
      logger.error('UPS tracking failed', { error: error.message, trackingNumber });
      throw new AppError('Failed to track UPS shipment', 500);
    }
  }
}
...
export class USPSCarrier implements ShippingCarrier {
  ...
  async getRates(request: ShippingRateRequest): Promise<ShippingRate[]> {
    try {
      ...
      logger.error('USPS rate request failed', { error: error.message, request });
      throw new AppError('Failed to get USPS rates', 500);
    }
  }
  ...
  async createShipment(request: ShipmentRequest): Promise<ShipmentResponse> {
    try {
      ...
      logger.error('USPS shipment creation failed', { error: error.message, request });
      return {
        success: false,
        rate: 0,
        currency: 'USD',
  ...
  async trackShipment(trackingNumber: string): Promise<TrackingInfo> {
    try {
      ...
      logger.error('USPS tracking failed', { error: error.message, trackingNumber });
      throw new AppError('Failed to track USPS shipment', 500);
    }
  }
}
...
export class ShippingService {
  ...
  getCarrier(name?: string): ShippingCarrier {
    ...
    if (!carrier) {
      throw new AppError(`Shipping carrier '${carrierName}' not configured`, 500);
    }
    
    return carrier;
  }

  async getRates(request: ShippingRateRequest, carriers?: string[]): Promise<ShippingRate[]> {
  ...
  async cancelShipment(shipmentId: string, carrier: string): Promise<boolean> {
    ...
    throw new AppError(`Carrier '${carrier}' does not support shipment cancellation`, 400);
  }

  async validateAddress(address: ShippingAddress, carrier?: string): Promise<ShippingAddress> {
    const shippingCarrier = this.getCarrier(carrier);
  ...
  private detectCarrierFromTrackingNumber(trackingNumber: string): string {
    ...
    if (trackingNumber.startsWith('1Z')) {
      ...
      return 'usps';
    } else if (trackingNumber.match(/^\d{12}$/) || trackingNumber.match(/^\d{14}$/)) {
      return 'fedex';
    }
   ...
   */
  async getFastestRate(request: ShippingRateRequest): Promise<ShippingRate | null> {
    const rates = await this.getRates(request);
  ...
  calculateDimensionalWeight(dimensions: { length: number; width: number; height: number }): number {
    ...
    return Math.ceil((dimensions.length * dimensions.width * dimensions.height) / 5000);
  }
