...
import { OrderStatus, FinancialStatus, FulfillmentStatus, PaymentMethod, PaymentStatus } from '@prisma/client';
...
describe('Order Management System - Basic Tests', () => {
  ...
  describe('Order Validation Rules', () => {
    it('should validate order item quantities', () => {
      ...
      const returnableQuantity = orderItem.quantityFulfilled - orderItem.quantityReturned;
  ...
  describe('Order State Transitions', () => {
    it('should allow valid order status transitions', () => {
      ...
      expect(validTransitions[OrderStatus.PENDING]).toContain(OrderStatus.CONFIRMED);
      expect(validTransitions[OrderStatus.CONFIRMED]).toContain(OrderStatus.PROCESSING);
      expect(validTransitions[OrderStatus.PROCESSING]).toContain(OrderStatus.SHIPPED);
      expect(validTransitions[OrderStatus.SHIPPED]).toContain(OrderStatus.DELIVERED);
    });
    ...
    it('should prevent invalid order status transitions', () => {
      ...
      const isValidTransition = validTransitions[currentStatus]?.includes(attemptedStatus) || false;
  ...
  describe('Fulfillment Logic', () => {
    it('should determine fulfillment status based on items', () => {
      ...
      const allFulfilled = orderItems.every(item => item.quantityFulfilled >= item.quantity);
      const partiallyFulfilled = orderItems.some(item => item.quantityFulfilled > 0);
