...
import { mockSyncResult, mockSyncStatus, mockWebhookEvent } from './mocks/shopify-mocks';
  ...
  describe('Sync Status Routes', () => {
    ...
    describe('GET /sync/history', () => {
      it('should get sync history successfully', async () => {
        ...
        expect(mockShopifyService.getSyncHistory).toHaveBeenCalledWith('products');
      });
    });
    ...
    describe('GET /sync/metrics', () => {
      it('should get sync metrics successfully', async () => {
        ...
        vi.mocked(SyncStatusManager.prototype.getSyncMetrics).mockResolvedValue(metrics);
