...
import { mockShopifyProduct, mockShopifyOrder, mockShopifyCustomer } from './mocks/shopify-mocks';
...
describe('ShopifyService', () => {
  ...
  describe('Circuit Breaker', () => {
    it('should open circuit breaker after failures', async () => {
      ...
      mockPrisma.syncStatus.create.mockResolvedValue({ id: 'sync-1' });
      mockPrisma.product.findMany.mockResolvedValue([
        { id: '1', syncStatus: 'pending', variants: [], images: [], collections: [] },
      ]);
      ...
      // Act & Assert
      await expect(shopifyService.syncProductsToShopify()).resolves.toBeDefined();
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', async () => {
      ...
      // Act
      await expect(shopifyService.syncProductsFromShopify()).rejects.toThrow('Network Error');
    ...
    it('should handle authentication errors', async () => {
      ...
      // Act
      await expect(shopifyService.syncProductsFromShopify()).rejects.toThrow('Unauthorized');
    });
  });
