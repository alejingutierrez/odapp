...
describe('ActivityIcon', () => {
  ...
  it('applies variant classes correctly', () => {
    const { container } = render(
      <ActivityIcon type='message' variant='success' />
    )
  ...
  it('shows background when showBackground is true', () => {
    ...
    expect(container.firstChild).toHaveClass(
      'oda-activity-icon--with-background'
    )
  })
  ...
  it('applies clickable class when onClick is provided', () => {
    ...
    const { container } = render(
      <ActivityIcon type='call' onClick={handleClick} />
    )
  ...
  it('does not handle click when disabled', () => {
    const handleClick = vi.fn()
    const { container } = render(
      <ActivityIcon type='call' onClick={handleClick} disabled />
    )
  ...
  it('applies custom background color', () => {
    const { container } = render(
      <ActivityIcon type='call' showBackground backgroundColor='#ff0000' />
    )
  ...
  it('applies custom className', () => {
    const { container } = render(
      <ActivityIcon type='call' className='custom-class' />
    )
  ...
  describe('activity types', () => {
    ...
    activityTypes.forEach((type) => {
      it(`renders ${type} activity type correctly`, () => {
        ...
        expect(
          container.querySelector('.oda-activity-icon')
        ).toBeInTheDocument()
      })
    })
  })
  ...
  describe('variant colors', () => {
    it('applies primary variant color', () => {
      const { container } = render(
        <ActivityIcon type='call' variant='primary' />
      )
    ...
    it('applies success variant color', () => {
      const { container } = render(
        <ActivityIcon type='call' variant='success' />
      )
    ...
    it('applies warning variant color', () => {
      const { container } = render(
        <ActivityIcon type='call' variant='warning' />
      )
