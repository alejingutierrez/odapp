...
describe('CurrencyDisplay', () => {
  ...
  it('renders different currencies correctly', () => {
    const { rerender } = render(<CurrencyDisplay amount={1000} currency="EUR" />);
  ...
  describe('currency icons', () => {
    ...
    it('hides currency icon when showIcon is false', () => {
      const { container } = render(<CurrencyDisplay amount={1000} showIcon={false} />);
      expect(container.querySelector('.anticon')).not.toBeInTheDocument();
    });

    it('shows different icons for different currencies', () => {
      const { container: usdContainer } = render(<CurrencyDisplay amount={1000} currency="USD" showIcon />);
      const { container: eurContainer } = render(<CurrencyDisplay amount={1000} currency="EUR" showIcon />);
