...
export type DateFormatType = 'short' | 'medium' | 'long' | 'relative' | 'time' | 'datetime' | 'smart';
...
export const DateDisplay: React.FC<DateDisplayProps> = ({
  ...
  const formatDate = (dateToFormat: Date, formatType: DateFormatType): string => {
    if (formatType === 'relative') {
      ...
      if (diffInDays < 0 && diffInDays > -7) return `In ${Math.abs(diffInDays)}d`;
    ...
    if (formatType === 'smart') {
      ...
      if (Math.abs(diffInDays) < 365) {
        return new Intl.DateTimeFormat(locale, { month: 'short', day: 'numeric' }).format(dateToFormat);
      }
      return new Intl.DateTimeFormat(locale, { year: 'numeric', month: 'short', day: 'numeric' }).format(dateToFormat);
    }
  ...
  const getDateIcon = () => {
    ...
    if (format === 'relative' || (format === 'smart' && Math.abs(diffInDays) < 7)) {
  ...
  const getContainerStyle = (): CSSProperties => {
    ...
    switch (variant) {
      case 'compact':
        ...
        break;
      case 'badge':
        baseStyle.background = 'linear-gradient(135deg, #f0f2f5 0%, #e6f7ff 100%)';
  ...
  const content = (
    <span style={getContainerStyle()}>
      {getDateIcon() && (
        <span style={getIconStyle()}>{getDateIcon()}</span>
      )}
      <Text 
