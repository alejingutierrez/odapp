...
describe('ProgressIndicator', () => {
  ...
  it('renders different variants correctly', () => {
    const { rerender } = render(
      <ProgressIndicator percent={50} variant='circle' />
    )
  ...
  it('does not render label when showLabel is false', () => {
    render(
      <ProgressIndicator percent={50} label='Loading...' showLabel={false} />
    )
  ...
  it('applies correct size classes', () => {
    const { container, rerender } = render(
      <ProgressIndicator percent={50} size='small' />
    )
  ...
  it('applies animated status when animated is true', () => {
    ...
    expect(
      document.querySelector('.ant-progress-status-active')
    ).toBeInTheDocument()
  })
})
