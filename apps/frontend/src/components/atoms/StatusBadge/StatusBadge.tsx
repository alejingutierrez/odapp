...
export type StatusType = 'success' | 'processing' | 'warning' | 'error' | 'default' | 'info' | 'pending' | 'cancelled';
export type StatusSize = 'small' | 'default' | 'large';
export type StatusVariant = 'badge' | 'dot' | 'outlined' | 'filled' | 'minimal';

export interface StatusBadgeProps extends Omit<BadgeProps, 'status' | 'color' | 'size'> {
  status: StatusType;
  size?: StatusSize;
...
export const StatusBadge: React.FC<StatusBadgeProps> = ({
  ...
  const getStatusColor = () => {
    switch (status) {
      ...
      case 'pending': return token.colorTextSecondary;
      case 'cancelled': return token.colorTextDisabled;
  ...
  const getStatusIcon = () => {
    ...
    switch (status) {
      ...
      case 'processing': return <SyncOutlined spin={animated} {...iconProps} />;
      case 'warning': return <ExclamationCircleOutlined {...iconProps} />;
      case 'error': return <CloseCircleOutlined {...iconProps} />;
      case 'info': return <InfoCircleOutlined {...iconProps} />;
      case 'pending': return <ClockCircleOutlined {...iconProps} />;
      case 'cancelled': return <MinusCircleOutlined {...iconProps} />;
