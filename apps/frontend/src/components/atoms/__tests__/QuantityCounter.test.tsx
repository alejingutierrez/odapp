...
describe('QuantityCounter', () => {
  ...
  it('validates against stock', () => {
    const handleChange = vi.fn()
    const { container } = render(<QuantityCounter value={15} stock={10} validateOnChange onChange={handleChange} />)
    
    // Check that input shows out-of-range class
    expect(container.querySelector('.ant-input-number-out-of-range')).toBeInTheDocument()
  })

  it('renders compact variant', () => {
    const { container } = render(<QuantityCounter variant="compact" />)
    expect(container.querySelector('.oda-quantity-counter--compact')).toBeInTheDocument()
  })

  it('renders inline variant', () => {
    const { container } = render(<QuantityCounter variant="inline" />)
    expect(container.querySelector('.oda-quantity-counter--inline')).toBeInTheDocument()
    expect(screen.getByText('Qty:')).toBeInTheDocument()
  })

  it('renders stepper variant', () => {
    const { container } = render(<QuantityCounter variant="stepper" />)
    expect(container.querySelector('.oda-quantity-counter--stepper')).toBeInTheDocument()
  })
  ...
  it('handles bulk entry mode', () => {
    ...
    expect(screen.getByRole('button', { name: /bulk entry/i })).toBeInTheDocument()
  })

  it('applies size classes correctly', () => {
    const { container } = render(<QuantityCounter size="large" />)
    expect(container.querySelector('.oda-quantity-counter--large')).toBeInTheDocument()
  })
