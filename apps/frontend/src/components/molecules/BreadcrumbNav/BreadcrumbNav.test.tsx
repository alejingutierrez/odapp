...
describe('BreadcrumbNav', () => {
  ...
  it('shows home item by default', () => {
    ...
    expect(screen.getByText('Home')).toBeInTheDocument()
    expect(screen.getByTestId('homeoutlined-icon')).toBeInTheDocument()
  })
  ...
  it('shows ellipsis dropdown when items exceed maxItems', () => {
    ...
    expect(
      document.querySelector('.breadcrumb-nav__ellipsis')
    ).toBeInTheDocument()
  })
  ...
  it('uses custom home href', () => {
    render(
      <BreadcrumbNav items={mockItems.slice(0, 1)} homeHref='/dashboard' />
    )
  ...
  it('renders with custom separator', () => {
    render(<BreadcrumbNav items={mockItems.slice(0, 2)} separator='>' />)
