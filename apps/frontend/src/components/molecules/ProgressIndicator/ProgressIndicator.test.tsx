...
describe('ProgressIndicator', () => {
  ...
  it('applies different statuses', () => {
    const { rerender } = render(
      <ProgressIndicator percent={100} status='success' />
    )

    expect(
      document.querySelector('.ant-progress-status-success')
    ).toBeInTheDocument()

    rerender(<ProgressIndicator percent={50} status='exception' />)
    expect(
      document.querySelector('.ant-progress-status-exception')
    ).toBeInTheDocument()
  })

  it('applies custom className', () => {
    render(<ProgressIndicator percent={40} className='custom-progress' />)
