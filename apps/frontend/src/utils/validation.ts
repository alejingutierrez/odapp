...
yup.addMethod(yup.string, 'hexColor', function (message = 'Invalid hex color format') {
  return this.matches(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, message)
})

yup.addMethod(yup.string, 'currency', function (message = 'Invalid currency code') {
  return this.matches(/^[A-Z]{3}$/, message)
})

yup.addMethod(yup.string, 'strongPassword', function (message = 'Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character') {
  return this.matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/, message)
})
...
export const productSchemas = {
  ...
  variant: yup.object({
    ...
    size: yup.string().required('Size is required').max(20, 'Size must be less than 20 characters'),
    color: yup.string().required('Color is required').max(50, 'Color must be less than 50 characters'),
    colorHex: commonSchemas.hexColor,
    material: yup.string().max(100, 'Material must be less than 100 characters').nullable(),
    ...
    barcode: yup.string().max(50, 'Barcode must be less than 50 characters').nullable(),
    inventoryQuantity: yup.number().integer().min(0, 'Inventory quantity must be non-negative').required(),
  ...
  image: yup.object({
    ...
    altText: yup.string().max(255, 'Alt text must be less than 255 characters').nullable(),
    position: yup.number().integer().min(0, 'Position must be non-negative').default(0),
  }),
  
  seo: yup.object({
    title: yup.string().max(60, 'SEO title must be less than 60 characters').nullable(),
    description: yup.string().max(160, 'SEO description must be less than 160 characters').nullable(),
    keywords: yup.array().of(yup.string()).max(10, 'Maximum 10 keywords allowed').nullable(),
  }),
}
...
export const customerSchemas = {
  ...
  address: yup.object({
    type: yup.string().oneOf(['billing', 'shipping', 'both'], 'Invalid address type').default('both'),
    firstName: yup.string()
      .min(1, 'First name is required')
      .max(50, 'First name must be less than 50 characters')
      .required('First name is required'),
    lastName: yup.string()
      ...
      .max(50, 'Last name must be less than 50 characters')
      .required('Last name is required'),
    company: yup.string().max(100, 'Company must be less than 100 characters').nullable(),
    address1: yup.string().required('Address line 1 is required').max(255, 'Address line 1 must be less than 255 characters'),
    address2: yup.string().max(255, 'Address line 2 must be less than 255 characters').nullable(),
    city: yup.string().required('City is required').max(100, 'City must be less than 100 characters'),
    province: yup.string().max(100, 'Province must be less than 100 characters').nullable(),
    country: yup.string().length(2, 'Country must be 2 characters').required('Country is required'),
    zip: yup.string().required('ZIP code is required').max(20, 'ZIP code must be less than 20 characters'),
    phone: commonSchemas.phone,
    isDefault: yup.boolean().default(false),
  }),
  
  preferences: yup.object({
    language: yup.string().matches(/^[a-z]{2}(-[A-Z]{2})?$/, 'Invalid language format').default('en'),
...
export const orderSchemas = {
  ...
  lineItem: yup.object({
    ...
    title: yup.string().required('Product title is required').max(255, 'Title must be less than 255 characters'),
    variantTitle: yup.string().max(255, 'Variant title must be less than 255 characters').nullable(),
  ...
  discount: yup.object({
    code: yup.string().max(50, 'Discount code must be less than 50 characters').nullable(),
    type: yup.string().oneOf(['percentage', 'fixed_amount', 'shipping'], 'Invalid discount type').required(),
    value: commonSchemas.positiveNumber.required('Discount value is required'),
    amount: commonSchemas.positiveNumber.required('Discount amount is required'),
    title: yup.string().max(255, 'Discount title must be less than 255 characters').required(),
  }),
  
  shippingLine: yup.object({
    title: yup.string().required('Shipping title is required').max(255, 'Title must be less than 255 characters'),
    price: commonSchemas.positiveNumber.required('Shipping price is required'),
    code: yup.string().max(50, 'Shipping code must be less than 50 characters').nullable(),
    carrier: yup.string().max(100, 'Carrier must be less than 100 characters').nullable(),
  }),
  
  status: yup.string()
    .oneOf(['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded', 'partially_refunded'], 'Invalid order status')
    .default('pending'),
  
  paymentStatus: yup.string()
    .oneOf(['pending', 'authorized', 'paid', 'partially_paid', 'refunded', 'partially_refunded', 'voided', 'failed'], 'Invalid payment status')
    .default('pending'),
  
  fulfillmentStatus: yup.string()
    .oneOf(['unfulfilled', 'partial', 'fulfilled', 'restocked'], 'Invalid fulfillment status')
    .default('unfulfilled'),
}
...
export const authSchemas = {
  login: yup.object({
    ...
    twoFactorCode: yup.string().matches(/^\d{6}$/, 'Two-factor code must be 6 digits').nullable(),
  }),
...
export const zodToYup = (zodSchema: z.ZodSchema): yup.Schema<any> => {
