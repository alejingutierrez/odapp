...
export const customerAddressSchema = z.object({
  ...
  company: z.string().max(100, 'Company must be less than 100 characters').optional().transform((val) => val ? sanitizeString(val) : val),
  address1: z.string().min(1, 'Address line 1 is required').max(255, 'Address line 1 must be less than 255 characters').transform(sanitizeString),
  address2: z.string().max(255, 'Address line 2 must be less than 255 characters').optional().transform((val) => val ? sanitizeString(val) : val),
  city: z.string().min(1, 'City is required').max(100, 'City must be less than 100 characters').transform(sanitizeString),
  province: z.string().max(100, 'Province must be less than 100 characters').optional().transform((val) => val ? sanitizeString(val) : val),
  country: z.string().min(2, 'Country is required').max(2, 'Country must be 2 characters').regex(/^[A-Z]{2}$/, 'Country must be 2 uppercase letters'),
...
export const customerLoyaltySchema = z.object({
  ...
  lifetimeValue: z.number().min(0, 'Lifetime value must be non-negative').default(0),
})
...
export const customerSchema = z.object({
  ...
  addresses: z.array(customerAddressSchema).max(10, 'Maximum 10 addresses allowed').default([]),
  preferences: customerPreferencesSchema.default({}),
  loyalty: customerLoyaltySchema.default({}),
  segmentIds: z.array(uuidSchema).max(20, 'Maximum 20 segments allowed').default([]),
