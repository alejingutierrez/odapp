...
export const productVariantSchema = z.object({
  ...
  inventoryQuantity: z.number().int().min(0, 'Inventory quantity must be non-negative').default(0),
...
export const productSchema = z.object({
  ...
  productType: z.string().max(100, 'Product type must be less than 100 characters').optional(),
  tags: z.array(z.string().max(50, 'Tag must be less than 50 characters')).max(20, 'Maximum 20 tags allowed').default([]),
  images: z.array(productImageSchema).max(10, 'Maximum 10 images allowed').default([]),
  variants: z.array(productVariantSchema).min(1, 'At least one variant is required'),
  categoryId: uuidSchema.optional(),
  collectionIds: z.array(uuidSchema).max(10, 'Maximum 10 collections allowed').default([]),
  seo: productSeoSchema.optional(),
